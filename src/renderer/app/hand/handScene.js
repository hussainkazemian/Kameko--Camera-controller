...
export class HandScene {
    constructor(canvas) {
        ...
        this.camera = new THREE.PerspectiveCamera(70, canvas.clientWidth / canvas.clientHeight, 0.1, 2000);
    ...
    updateHand(landmarks, handedness) {
        const mats = this.materials[(handedness || 'unknown').toLowerCase()] || this.materials.unknown;
        ...
        for (const [a, b] of HAND_CONNECTIONS) {
            ...
            positions.push(
                mapX(la.x), mapY(la.y), (la.z || 0) * zScale,
                mapX(lb.x), mapY(lb.y), (lb.z || 0) * zScale
            );
        }
